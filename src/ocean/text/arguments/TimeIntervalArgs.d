/*******************************************************************************
    ...
    Timerange rounded
        Gives X amount in the past NOT including the current second, rounded.
...
public TimestampInterval processTimeIntervalArgs ( Arguments args )
{
    ...
    if ( num_args == 1 )
    {
        ...
        if ( isTimeInterval(arg) )
        {
            ...
            {
                long modulu_this;
                timeUnit(arg, modulu_this);
                end = end - (end % modulu_this);
            }
...
unittest
{
    ...
    Test.test!("==")(modulu_this, 1);

    timeUnit("5m", modulu_this);
    Test.test!("==")(modulu_this, 60);

    timeUnit("7h", modulu_this);
    Test.test!("==")(modulu_this, 3600);

    timeUnit("1d", modulu_this);
    Test.test!("==")(modulu_this, 86400);
}
